...
def NN_SA(file_name, classifier_col):
    ...
    while 1 == 1:
        ...
        for iter_test in iters:
            ...
            print('loss', nn_loss)
            outcome = {'schedule': 'GeomDecay', 'activation': act, 'learning_rate': lr, 'max_iters': iter_test,
                       'temperatures': temp, 'decay_rates': dec, 'max_attempts': ma, 'clip': clip,
                       'y_train_roc': y_train_roc, 'y_test_roc': y_test_roc,
                       'runtime': train_time + y_train_query_time + y_test_query_time, 'Train time': train_time,
                       'y_train_query_time': y_train_query_time, 'y_test_query_time': y_test_query_time,
                       'loss': nn_loss}
        ...
        for temp_test in temperatures:
            ...
            outcome = {'schedule': 'GeomDecay', 'activation': act, 'learning_rate': lr, 'max_iters': itera,
                       'temperatures': temp_test, 'decay_rates': dec, 'max_attempts': ma, 'clip': clip,
        ...
        for decay_test in decay_rates:
            ...
            outcome = {'schedule': 'GeomDecay', 'activation': act, 'learning_rate': lr, 'max_iters': itera,
                       'temperatures': temp, 'decay_rates': decay_test, 'max_attempts': ma, 'clip': clip,
                       'y_train_roc': y_train_roc, 'y_test_roc': y_test_roc, 'runtime': runtime}
            outcomes.append(outcome)
            pd.DataFrame(outcomes).to_csv(csv_file)
        ...
        for clip_test in clips:
            ...
            outcome = {'schedule': 'GeomDecay', 'activation': act, 'learning_rate': lr, 'max_iters': itera,
                       'temperatures': temp, 'decay_rates': dec, 'max_attempts': ma, 'clip': clip_test,
                       'y_train_roc': y_train_roc, 'y_test_roc': y_test_roc, 'runtime': runtime}
            outcomes.append(outcome)
            pd.DataFrame(outcomes).to_csv(csv_file)
        ...
        for maxa_test in max_attempts:
            ...
            outcome = {'schedule': 'GeomDecay', 'activation': act, 'learning_rate': lr, 'max_iters': itera,
                       'temperatures': temp, 'decay_rates': dec, 'max_attempts': maxa_test, 'clip': clip,
                       'y_train_roc': y_train_roc, 'y_test_roc': y_test_roc, 'runtime': runtime}
            outcomes.append(outcome)
            pd.DataFrame(outcomes).to_csv(csv_file)
        ...
        for lr_test in learning_rate:
            ...
            outcome = {'schedule': 'GeomDecay', 'activation': act, 'learning_rate': lr_test, 'max_iters': itera,
                       'temperatures': temp, 'decay_rates': dec, 'max_attempts': ma, 'clip': clip,
                       'y_train_roc': y_train_roc, 'y_test_roc': y_test_roc, 'runtime': runtime}
            outcomes.append(outcome)
            pd.DataFrame(outcomes).to_csv(csv_file)
